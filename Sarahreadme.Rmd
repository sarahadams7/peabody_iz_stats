...
US_ExEx_Best_Location <- U.S._ExEx %>% select('Catalog Number', Country, 'City/Town/Nearest Named Place', 'Latitude (dec)', 'Longitude (dec)')
...
Multiple files for Seychelles expedition of 1957-58, going to combine them below.

```{r}
seychelles_string = "Seychelles"

collectors_focal = 
	collectors_frame %>%
	filter( str_detect(Name, seychelles_string) )

collectors_focal

sm = D_ll %>% filter(`Collector/Expedition` %in% collectors_focal$Name[1:10]) %>%
  ggplot() +  
		mapWorld +
		geom_point(aes(x=`Longitude (dec)`, y=`Latitude (dec)`), alpha = 0.2, size=1) 

sm
	
seycheles_Locality <- seychelles_Best_Location %>% select('Catalog Number', Locality, 'Latitude (dec)', 'Longitude (dec)')
seychelles_Locality
